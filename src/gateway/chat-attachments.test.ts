...
describe("parseMessageWithAttachments", () => {
  ...
  it("collects non-image payloads as files and logs", async () => {
  ...
  it("collects unknown mime as file when sniff fails and logs", async () => {
    ...
    expect(logs).toHaveLength(1);
    expect(logs[0]).toMatch(/collecting as file/i);
  });

  it("collects CSV attachment as file", async () => {
    const csv = Buffer.from("Name,Value\nfoo,1\nbar,2").toString("base64");
    const { parsed, logs } = await parseWithWarnings("analyze this", [
      {
        type: "file",
        mimeType: "text/csv",
        fileName: "data.csv",
    ...
    expect(parsed.images).toHaveLength(0);
    expect(parsed.files).toHaveLength(1);
    expect(parsed.files[0]?.fileName).toBe("data.csv");
    expect(parsed.files[0]?.type).toBe("file");
    expect(logs).toHaveLength(1);
    expect(logs[0]).toMatch(/collecting as file/i);
  });

  it("keeps valid images and collects non-images as files", async () => {
    const pdf = Buffer.from("%PDF-1.4\n").toString("base64");
    const { parsed, logs } = await parseWithWarnings("x", [
      {
        type: "image",
